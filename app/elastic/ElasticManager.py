...
class ElasticManager:
    def __init__(
        self,
        elastic_host=cfg["elastic"]["host"],
        elastic_index=cfg["elastic"]["ingestion_index"],
    ) -> None:
    ...
    def scan(self, query):
        scan = helpers.scan(
            self.es, query=query, index=self.elastic_index, preserve_order=True
        )
        return scan
    ...
    def build_jsonl(batch_size=1_000_000):
        ...
        query = {
            "function_score": {
                "query": {"bool": {"must_not": {"exists": {"field": "answer"}}}},
                "functions": [
                    {"random_score": {"seed": f"{query_seed}", "field": "_seq_no"}}
                ],
                "boost_mode": "replace",
            }
        }
