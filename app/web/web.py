...
def training_log():
    ...
    with open(
        os.path.abspath(f"{cfg['folders']['output']}/prodigy.train.log"), "r"
    ) as f:
        logdata = f.read()

    with open(
        os.path.abspath(f"{cfg['folders']['output']}/prodigy.train-curve.log"), "r"
    ) as f:
        curvedata = f.read()
    ...
    return render_template(
        "training-log.html", logdata=html_log, curvedata=html_curve_log
    )
    ...
    if request.method == "POST":
        for file in request.files.getlist("files"):
            file_name = os.path.join(
                cfg["folders"]["watch_folder"], secure_filename(file.filename)
            )
