...
class contentProcessor:
    ...
    def process(self, body: str, file, timestamp):
        ...
        if self.alreadyIngested(file_id) is True:
            logging.info(
                f"Skipping {file} (id: {file_id}) that already exists in the database"
            )
            return
        ...
        for doc in docs:
            ...
            line["body"]["meta"]["id"] = str(
                sha1((str(file) + str(sent)).encode("utf-8")).hexdigest()
            )
            line["body"]["meta"]["ents"] = {}
            ...
            for ent in doc.ents:
                ...
                try:
                    if isinstance(
                        line["body"]["meta"]["ents"][label], list
                    ) and isinstance(line["body"]["meta"]["ents"][label_int], list):
    ...
    def alreadyIngested(self, source_hash) -> bool:
        query = {
            "size": 1,
            "query": {
                "bool": {"must": [{"match": {"meta.source_hash": f"{source_hash}"}}]}
            },
        }
